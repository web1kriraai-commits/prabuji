...
const MultiStepRegistrationForm = ({ yatra, onClose }) => {
        ...
        if (savedData) {
            try {
                ...
                if (parsed.selectedPackages !== undefined) {
                    ...
                    setSelectedPackages(parsed.selectedPackages);
                } else if (parsed.selectedPackage) {
                    // Backward compatibility
    ...
    const yatraDuration = (() => {
        if (!yatra.startDate || !yatra.endDate) return 1;
        const start = new Date(yatra.startDate);
        const end = new Date(yatra.endDate);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const advancePerPerson = parseFloat(yatra.advancePaymentAmount);
            if (!isNaN(advancePerPerson) && advancePerPerson > 0) {
                const advanceAmount = Math.round(advancePerPerson * members.length);
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-3">
                            ...
                            <div className="border-t-2 border-dashed border-gray-200 pt-3 mt-4">
                                ...
                                {(!isNaN(parseFloat(yatra.advancePaymentAmount)) && parseFloat(yatra.advancePaymentAmount) > 0) && (
                                    <div className="mt-3 bg-orange-50 border border-orange-200 rounded-lg p-3">
                                        <div className="flex justify-between items-center text-base mb-1">
                                            <span className="font-semibold text-orange-800">Advance Payment Required</span>
                                            <span className="font-bold text-orange-700">
                                                ₹{(parseFloat(yatra.advancePaymentAmount) * members.length).toLocaleString()}
                                            </span>
                                        </div>
                                        <p className="text-xs text-orange-600">
                                            * Pay ₹{parseFloat(yatra.advancePaymentAmount).toLocaleString()} per person (Total: ₹{(parseFloat(yatra.advancePaymentAmount) * members.length).toLocaleString()}) to confirm booking
                                        </p>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                            ...
                            <p className="text-sm text-blue-700 mb-2">
                                Please pay <span className="font-bold">
                                    {(!isNaN(parseFloat(yatra.advancePaymentAmount)) && parseFloat(yatra.advancePaymentAmount) > 0)
                                        ? `₹${(parseFloat(yatra.advancePaymentAmount) * members.length).toLocaleString()} (Advance Amount)`
